-- Feature 1: keep private admin histories for 31 days.
...
declare
  ...
  v_count bigint;
begin
  delete from public.collector_run_history where stopped_at < now() - interval '31 days';
  get diagnostics v_count = row_count; v_deleted := v_deleted + v_count;
  delete from public.store_scan_history where started_at < now() - interval '31 days';
  get diagnostics v_count = row_count; v_deleted := v_deleted + v_count;
  delete from public.diagnostic_events where created_at < now() - interval '31 days';
  get diagnostics v_count = row_count; v_deleted := v_deleted + v_count;
  return v_deleted;
end;
$$;
