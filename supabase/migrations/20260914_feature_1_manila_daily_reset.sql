-- Feature 1: unchanged products become due at the next Asia/Manila midnight.
create or replace function public.mark_product_check(
  p_product_id bigint, p_checked_date date, p_checked_at timestamptz, p_source text, p_status text,
  p_variation_count integer, p_changed_count integer, p_unchanged_count integer, p_failed_count integer,
...
begin
  ...
  select exists(select 1 from public.product_daily_checks where product_id=p_product_id
    and checked_date=p_checked_date and status='success') into v_already_successful_today;

  insert into public.product_daily_checks(product_id,checked_date,checked_at,source,status,
    variation_count,changed_count,unchanged_count,failed_count,metadata)
  values(p_product_id,p_checked_date,p_checked_at,p_source,p_status,
    greatest(0,least(coalesce(p_variation_count,0),200)), greatest(0,least(coalesce(p_changed_count,0),200)),
    greatest(0,least(coalesce(p_unchanged_count,0),200)), greatest(0,least(coalesce(p_failed_count,0),200)),
    coalesce(p_metadata,'{}'::jsonb))
  on conflict(product_id,checked_date) do update set
    checked_at=greatest(product_daily_checks.checked_at,excluded.checked_at),
  ...
  update public.products set
    ...
    consecutive_sold_out_checks=case
      ...
      when p_status='success' and v_all_sold_out and not v_already_successful_today then consecutive_sold_out_checks+1
      else consecutive_sold_out_checks end,
    ...
    next_check_at=case
      when p_status='success' and v_all_sold_out and not v_skip_sold_out then
        (date_trunc('day',p_checked_at at time zone 'Asia/Manila')+interval '1 day') at time zone 'Asia/Manila'
      when p_status='success' and v_all_sold_out and v_skip_sold_out and v_already_successful_today then next_check_at
      when p_status='success' and v_all_sold_out and v_skip_sold_out and consecutive_sold_out_checks>=2 then p_checked_at+interval '30 days'
      when p_status='success' and v_all_sold_out and v_skip_sold_out then p_checked_at+interval '15 days'
