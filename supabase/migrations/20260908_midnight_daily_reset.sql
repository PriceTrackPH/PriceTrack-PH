-- Reset regular available-product collection at 12:00 AM Asia/Manila
-- while preserving the escalating sold-out schedule.
...
begin
  ...
  set last_check_attempt_at = p_checked_at,
      ...
      next_check_at = case
        when p_status = 'success' and v_all_sold_out and v_already_successful_today
          then next_check_at
        when p_status = 'success' and v_all_sold_out and consecutive_sold_out_checks >= 2
...
-- Products successfully checked since today's Manila midnight wait for the
-- next reset. All other available products become due at today's reset.
