...
function safeDelay() {
  const value = Number(process.env.PRICETRACK_PRODUCT_DELAY_SECONDS || 60);
  return Number.isFinite(value) && value >= 30 ? Math.round(value * 1000) : 60_000;
}
...
async function main() {
  ...
  const state = { mode: "idle", message: "Ready", attempted: 0, succeeded: 0, failed: 0, consecutiveFailures: 0, afterProductId: 0, currentProduct: null, nextAt: 0, limit: Number.isFinite(limit) ? limit : null };
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/state") return sendJson(response, 200, state);
      if (request.method === "POST" && url.pathname === "/start") {
        await releaseCurrent(); Object.assign(state, { mode: "running", message: "Starting", attempted: 0, succeeded: 0, failed: 0, consecutiveFailures: 0, afterProductId: 0, nextAt: 0 }); return sendJson(response, 200, state);
      }
      if (request.method === "POST" && url.pathname === "/claim") {
        ...
        const claim = await apiRequest(site, token, "claim", { afterProductId: state.afterProductId });
        if (!claim.product) { state.mode = "finished"; state.message = "No more due products"; return sendJson(response, 200, { product: null, state }); }
        state.currentProduct = claim.product; state.afterProductId = claim.product.productId; state.attempted += 1; state.message = `Opening ${claim.product.shopId}.${claim.product.externalProductId}`; return sendJson(response, 200, { product: claim.product, state });
      }
      if (request.method === "POST" && url.pathname === "/observations") {
        ...
        const result = await apiRequest(site, token, "record", { payload: observation });
