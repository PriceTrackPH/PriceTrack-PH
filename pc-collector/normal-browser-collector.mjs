...
async function main() {
  ...
  const state = { mode: "idle", message: "Ready", attempted: 0, succeeded: 0, failed: 0, consecutiveFailures: 0, currentProduct: null, nextAt: 0, limit: Number.isFinite(limit) ? limit : null, totalTracked: 0, totalDue: 0, skipUnchangedDay: true };
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/start") {
        ...
        Object.assign(state, { mode: "running", message: "Starting", attempted: 0, succeeded: 0, failed: 0, consecutiveFailures: 0, nextAt: 0, totalTracked: summary.totalTracked, totalDue: summary.totalDue, skipUnchangedDay: options.skipUnchangedDay !== false }); return sendJson(response, 200, state);
      }
      ...
      if (request.method === "POST" && url.pathname === "/observations") {
        ...
        let result;
        try { result = await apiRequest(site, token, "record", { payload: { ...observation, skipUnchangedDay: state.skipUnchangedDay } }); }
