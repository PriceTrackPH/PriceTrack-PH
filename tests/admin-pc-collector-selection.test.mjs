...
test("status applies the collector checkbox when Shopee strips the URL setting", async () => {
  const originalFetch = global.fetch;
  const originalToken = process.env.ADMIN_HEALTH_TOKEN;
  const originalUrl = process.env.SUPABASE_URL;
  const originalSecret = process.env.SUPABASE_SECRET_KEY;
  ...
  global.fetch = async (url, options = {}) => {
    calls.push({ url: String(url), options });
    if (String(url).includes("product_daily_checks?") && options.method !== "PATCH") {
      return { ok: true, json: async () => [{
        id: 91,
        checked_at: "2026-09-07T12:40:18.971Z",
        metadata: { skip_unchanged_day: false, all_variations_unchanged: true },
      }] };
    }
    if (String(url).includes("products?id=eq.42") && options.method !== "PATCH") {
      return { ok: true, json: async () => [{ all_variations_sold_out: false, next_check_at: "2026-09-07T16:00:00.000Z" }] };
    }
    return { ok: true, json: async () => [] };
  };
  try {
...
test("status counts unchanged and sold-out products without showing defer dates when both toggles are off", async () => {
  const originalFetch = global.fetch;
  const originalToken = process.env.ADMIN_HEALTH_TOKEN;
  const originalUrl = process.env.SUPABASE_URL;
  const originalSecret = process.env.SUPABASE_SECRET_KEY;
  ...
  global.fetch = async (url, options = {}) => {
    ...
    if (String(url).includes("product_daily_checks?") && options.method !== "PATCH") {
      return { ok: true, json: async () => [{
        id: 91,
        checked_at: "2026-09-08T12:40:18.971Z",
        metadata: { all_variations_unchanged: !soldOut },
      }] };
    }
    if (String(url).includes("products?id=eq.42") && options.method !== "PATCH") {
      return { ok: true, json: async () => [{ all_variations_sold_out: soldOut, next_check_at: "2026-09-23T12:40:18.971Z" }] };
    }
    return { ok: true, json: async () => [] };
  };
  try {
    const requestStatus = async () => {
      ...
      await handler({
        method: "POST", headers: { authorization: "Bearer admin-token" },
        query: { action: "status" },
        body: { productId: 42, skipUnchangedDay: false, skipSoldOut: false },
      }, response);
      return responseBody;
    };

    const unchangedResult = await requestStatus();
    assert.equal(unchangedResult.samePrice, true);
    assert.equal(unchangedResult.samePriceRecheckAt, null);
    assert.equal(calls.some((call) => call.options.method === "PATCH"), false);

    soldOut = true;
    const soldOutResult = await requestStatus();
    assert.equal(soldOutResult.soldOut, true);
    assert.equal(soldOutResult.recheckAt, null);
  } finally {
    global.fetch = originalFetch;
    if (originalToken === undefined) delete process.env.ADMIN_HEALTH_TOKEN; else process.env.ADMIN_HEALTH_TOKEN = originalToken;
    if (originalUrl === undefined) delete process.env.SUPABASE_URL; else process.env.SUPABASE_URL = originalUrl;
